...
function showMicFallback() {
  ...
  // Replace the "listening" label with the hint
  setMicUI({ enabled: true, text: "Не получилось? кликни по экрану" });
  if (inlineTapHint) inlineTapHint.hidden = false;

  // Next user gesture should open the comic (iOS/WebAudio unlock)
  const openOnce = (e) => {
    if (!micFallbackShown) return;
    // Don't break normal button clicks (e.g., mic toggle); require a general tap/click
