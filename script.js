...
// iOS often needs a user gesture to unlock WebAudio.
// We'll try to resume AudioContext on the first touch/click.
...
// Blow detection: auto-calibrate baseline noise floor, then derive threshold.
...
function monitorMic() {
  ...
  const loop = (ts) => {
    ...
    if (micBaseline === null) {
      // start tracking baseline after mic is enabled
      micBaseline = { start: ts, sum: 0, n: 0 };
    }
    if (micBaseline && (ts - micBaseline.start) <= 800) {
      micBaseline.sum += rms;
      micBaseline.n += 1;
    } else if (micBaseline && micBaseline.n > 0 && micBaseline.avg === undefined) {
...
if (openComicBtn) {
  openComicBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    // allow opening comic even if mic doesn't work on iOS
    hideMicFallback();
    ensureAudioAwake();
    showComic();
  });
