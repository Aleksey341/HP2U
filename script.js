...
/* candle state */
function setStep(n){
  // n: 0 -> hide all, 1..3 -> show only that step
  const v = Number(n || 0);
...
function showMicFallback() {
  ...
  const openOnce = (e) => {
    if (!micFallbackShown) return;
    // ignore click on the mic toggle itself
...
cake.addEventListener("click", (e) => {
  ...
  // After the first cake click (prediction shown) -> show Step 2 button
  setStep(2);
...
if (stepMicBtn) stepMicBtn.addEventListener("click", (e) => {
  // Step 2 action -> enable mic
  setStep(3);
  toggleMic(e);
});
